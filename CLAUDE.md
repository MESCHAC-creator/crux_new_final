...
- **Bundle ID:** `com.schac_crux.app` (Android) / `com.schac-crux.app` (iOS)
- **Firebase project:** `crux-3c6be`
...
├── firebase_options.dart    # Firebase config (projectId: crux-3c6be)
...
| File | Role |
|---|---|
| `MainActivity.kt` | MethodChannels: `com.schac_crux.app/pip` (PiP 16:9), `com.schac_crux.app/screen_share`; auto-PiP on home button when inCall |
| `CallForegroundService.kt` | Foreground service (mic + camera types); channels: call (LOW) + screen share (HIGH); START_STICKY |

**BroadcastReceiver:** `com.schac_crux.app.STOP_SCREEN_SHARE_FROM_NOTIFICATION` → fires `stopScreenShareFromNotification` to Dart.
...
- **AGP:** 8.10.1 | **Gradle:** 8.12 | **Kotlin:** 2.1.0 | **Google Services:** 4.5.0
- **compileSdk / targetSdk:** 36 | **minSdk:** 21 | **NDK:** 27.0.12077973
- **Signing:** `android/key.jks` (alias: crux_alias, pass: Kouakou07)
- **Multidex:** enabled | **Minify/Shrink:** enabled
